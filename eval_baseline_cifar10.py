...
# net.linear = nn.Linear(net.linear.in_features, 4)
# net.load_state_dict(pretrained_rotation_dict, strict=False)
# net.linear = nn.Linear(net.linear.in_features, 10)

pretrained_simsiam_dict = {key.replace("backbone.", ""): value for key, value in simsiam['state_dict'].items()}
net.load_state_dict(pretrained_simsiam_dict, strict=False)
...
def test(epoch):
    ...
    with open('./best_simsiam_cifar10.txt','a') as f:
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/simsiam_cifar10.pth')
        best_acc = acc
