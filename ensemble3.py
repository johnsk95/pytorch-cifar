...
class weightMLP(nn.Module):
    def __init__(self, m1, m2, m3, num_tasks=3):
        ...
        self.weights = nn.Linear(512*2 + 2048, 512)
...
net = weightMLP(colorization_resnet, jigsaw_resnet, rotation_resnet)
net = net.to(device)
...
def test(epoch):
    ...
    with open('./best_ensemble3_rjc.txt','a') as f:
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/ensemble3_rjc.pth')
        best_acc = acc
