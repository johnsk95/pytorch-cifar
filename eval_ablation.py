...
from models.autoencoder_ablation import MergeAutoencoder
...
reconstruction = torch.load('./checkpoints/mergeColorJigsaw_autoencoder_500epochs.pth')
...
net = MergeAutoencoder(colorization_resnet, jigsaw_resnet)
...
def test(epoch):
    ...
    with open('./best_ablation_cj.txt','a') as f:
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/ablation_cj.pth')
        best_acc = acc
