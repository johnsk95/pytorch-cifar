...
from models.autoencoder_cs import MergeAutoencoder
...
net = MergeAutoencoder(colorization_resnet, simsiam_resnet)
...
def test(epoch):
    ...
    with open('./best_ablation_cs.txt','a') as f:
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/ablation_cs.pth')
        best_acc = acc
