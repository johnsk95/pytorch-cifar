...
class weightMLP(nn.Module):
    def __init__(self, m1, m2):
        ...
        self.weights = nn.Linear(512 + 2048, 512)
...
net = weightMLP(colorization_resnet, jigsaw_resnet)
net = net.to(device)
...
def test(epoch):
    ...
    with open('./best_ensemble2_jc.txt','a') as f:
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/ensemble2_jc.pth')
        best_acc = acc
